...
# SCATTERPLOT CODE
plt.scatter(mdf["InABR"], mdf["F1"], c=mdf["BCABR"],marker="o", cmap="Spectral", edgecolor="black", linewidth=0.7, alpha=0.8)
cbar = plt.colorbar()
cbar.set_label("log2(BC A/BC B)")
plt.xlabel("log2(InA/InB)")
plt.ylabel("F1")
plt.tight_layout()
plt.savefig("S2Bi.svg",dpi=400)
plt.savefig("S2Bi.png",dpi=400, pad_inches=0)
plt.show()
